...
const ChatBot = () => {
  const [messages, setMessages] = useState([{sender: "hoku", text: "Aloha! I am Hoku, your virtual assistant! How can I help you?"}]);
  const [text, setText] = useState("")
  const [hokuLoading, setHokuLoading] = useState(false);
  const [chatOpen, setChatOpen] = useState(false);
  const [show, setShow] = useState(false);

  const modalToggle = () => { setShow(true) }
    ...
    <div className="chat-container m-3 d-flex flex-column rounded-3 bg-white">
      ...
      <div className="overflow-auto flex-grow-1">
        <div className="d-flex flex-column">
          ...
          {hokuLoading && <AiChatMessage modalToggle={modalToggle} loading={ hokuLoading } text = { "loading..." } />}
        </div>
